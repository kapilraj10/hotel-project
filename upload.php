...
function handle_image_upload($fileInputName) {
    ...
    if ($file['error'] !== UPLOAD_ERR_OK) {
        $GLOBALS['LAST_UPLOAD_ERROR'] = 'Upload error code: ' . ($file['error'] ?? 'unknown');
        return null;
    }

    // Try multiple ways to detect mime type: finfo, then getimagesize as fallback
    // If uploads directory isn't writable, refuse uploads early.
    if (!defined('UPLOADS_WRITABLE') || UPLOADS_WRITABLE !== true) {
        $GLOBALS['LAST_UPLOAD_ERROR'] = 'Uploads directory is not writable on server.';
        log_upload('uploads-not-writable');
        return null;
    }
    ...
    if (!$mime) {
        $GLOBALS['LAST_UPLOAD_ERROR'] = 'Could not detect file MIME type.';
    ...
    if (!isset($map[$mime])) {
        $GLOBALS['LAST_UPLOAD_ERROR'] = 'File type not allowed: ' . ($mime ?? 'unknown');
    ...
    // move failed
    $GLOBALS['LAST_UPLOAD_ERROR'] = 'Failed to move uploaded file to destination.';
    log_upload('move-failed: dest=' . $dest . ' php_error=' . ($file['error'] ?? ''));
    return null;
}

function upload_last_error() {
    return $GLOBALS['LAST_UPLOAD_ERROR'] ?? null;
}
