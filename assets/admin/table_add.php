...
if ($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  try { $pdo->exec("ALTER TABLE tables_info ADD COLUMN IF NOT EXISTS price DECIMAL(10,2) DEFAULT 0"); } catch (Throwable $e) { /* ignore */ }
  ...
  $stmt=$pdo->prepare('INSERT INTO tables_info (table_number,table_type,status,image_path,price,is_public,capacity,bed_type,is_room) VALUES (?,?,?,?,?,?,?,?,?)');
  $stmt->execute([$tn,$tt,$st,$img,$price,$is_public,$capacity,$bed_type,0]);
...
<form method="post" enctype="multipart/form-data">
  ...
  <div class="mb-3"><label>Price per night (USD)</label><input name="price" class="form-control" type="number" step="0.01" min="0" value="0.00"></div>
  ...
  <div class="d-flex gap-2">
    ...
    <button name="action" value="save_and_book" class="btn btn-outline-primary">Save & Book</button>
  </div>
</form>
